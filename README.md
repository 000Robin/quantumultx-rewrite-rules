...
百度网盘开屏采用连接层精确拦截：2026-09-24 HAR 的两次冷启动均下载同一张 `fancydsp.oss-cn-beijing.aliyuncs.com/upload/ftx/advertiser/` 广告图片，并同时访问 Menta 的开屏适配器/投放活动接口及 ADN Plus 广告接口。管理分流只拒绝 `api-v3.mentamob.com`、`api-nxs-v4.mentamob.com`、`ad-api.adn-plus.com.cn` 和该精确素材主机；不拦截 `pan.baidu.com`、`diskapi.baidu.com`、`panpic.baidu.com` 或整个阿里云 OSS，不影响文件上传、下载、缩略图、账号和会员查询。该方案无需 MitM；首次更新后若仍显示旧素材，应清除 App 缓存或重新安装，因为开屏图片可能已落地缓存。
