/*
 * YouTube ad response cleaner for Surge.
 *
...
const binaryBody = responseBody instanceof Uint8Array ? responseBody : null;
const textBody = typeof responseBody === "string"
  ? responseBody
  : new TextDecoder().decode(binaryBody || new Uint8Array());
...
} else if (endpoint === "player" && binaryBody) {
  try {
    const output = stripPlayerAdFields(binaryBody);
    if (output) {
      $done({ body: output });
    } else {
      $done({});
